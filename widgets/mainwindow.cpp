  ...
  m_block_udp_status_updates {false}
{
  ...
  connect (m_messageClient, &MessageClient::setup_tx, [this] (int newTxMsgIdx, QString const& msg, bool param0, bool param1, QString const& check, quint32 offset) {    //avt 11/16/20
      if (!m_externalCtrl) {
        ...
        last_tx_label.setText("Controller initiated"); //avt 12/16/21
        debugToFile(QString{});
...
void MainWindow::callSandP2(int n)
{
  ...
  if(m_mode=="Q65" and m_specOp==SpecOp::Q65_PILEUP and n < 40) {
    ...
    genStdMsgs("-22");
    setTxMsg(3);
    if (!m_auto) ui->autoButton->click(); // Enable Tx
...
{
  //debugToFile("rdFrmStdout");   //avt 1/4/24
  ...
  while(proc_jt9.canReadLine()) {
    //debugToFile("rdFrmStdout(3)");   //avt 1/4/24
    ...
    //debugToFile("rdFrmStdout(4)");   //avt 1/4/24
    if (!(no_a7_decodes && line_read.contains("a7"))) {
    //debugToFile("rdFrmStdout(5)");   //avt 1/4/24
...
//      if(m_mode=="FT8" && fTR>0.6 && fTR<0.75) decodeDone();  // Clear a hung decoder status
      ...
      //debugToFile("rdFrmStdout(2)");   //avt 1/4/24
      if ("FST4W" == m_mode)
        {
        ...
        if (parts.size() > 6) {
          ...
          if(m_bCallingCQ && !m_bAutoReply && for_us && m_specOp!=SpecOp::FOX && m_specOp!=SpecOp::HOUND) {
            ...
            if(bProcessMsgNormally) {
              m_bDoubleClicked=true;
              m_bAutoReply = true;
              //debugToFile(QString{"rdFrmStdout: m_bAutoReply:%1 m_bCallingCQ:%2 message:%3"}.arg(m_bAutoReply).arg(m_bCallingCQ).arg(decodedtext.string()));   //avt 1/4/24
...
//    qDebug() << "AAA" << nsec%60 << int(m_specOp);
    ...
    if( SpecOp::HOUND == m_specOp ) {
      ...
      if(m_config.id_interval() > 0 and tHound >= m_config.id_interval() * 60 and m_ntx==1) {
      auto_tx_mode(false);
      statusUpdate ();
      }
    }
...
void MainWindow::startTx2()
{
  if (!m_modulator->isActive ()) { // TODO - not thread safe
    ...
    if((m_ntx==6 or m_ntx==7) and m_config.force_call_1st() and
      ...
      if (!is_externalCtrlMode()) ui->respondComboBox->setCurrentIndex(1);
    }
    transmit (snr);
...
// Make sure that MSK144 is not checked.
  ...
  //                                                  *
  //                       01234567890123456789012345678901234567
  displayWidgets(nWidgets("10111111010000000001000100001000000000"));	//to-do
  fast_config(m_bFastMode);
  statusChanged();
...
void MainWindow::postDecode (bool is_new, DecodedText decoded_text)      //avt 12/5/20
{
  ...
  if (((QDateTime::currentMSecsSinceEpoch() / 1000 - m_secBandChanged) > 4*int(m_TRperiod)/4)) {  //avt 7/7/22 band wasn't recently changed
    //debugToFile(QString {"enqueueDecode:  deCall:%1 m_mode:%2 grid:%3 m_currentBand:%4"}.arg(call).arg(m_mode).arg(grid).arg(m_currentBand));
    auto const& looked_up = m_logBook.countries ()->lookup (call);
    m_logBook.match (call, m_mode, grid, looked_up, callB4, countryB4, gridB4, continentB4, CQZoneB4, ITUZoneB4);
    //debugToFile(QString {"  callB4:%1 countryB4:%2 continentB4:%3"}.arg(callB4).arg(countryB4).arg(continentB4));
    m_logBook.match (call, m_mode, grid, looked_up, callB4onBand, countryB4onBand, gridB4onBand,
...
{
  //debugToFile("p1RdFrmStdout");   //avt 1/4/24
  QString t1;
...
void MainWindow::uploadWSPRSpots (bool direct_post, QString const& decode_text)
{
  ...
  auto pct = QString::number (m_auto ? ui->sbTxPercent->value () : 0);
  if (direct_post)
    {
...
void MainWindow::setCallPriority(QString call)
{
    bool callB4;
    bool callB4onBand;
    bool countryB4;
